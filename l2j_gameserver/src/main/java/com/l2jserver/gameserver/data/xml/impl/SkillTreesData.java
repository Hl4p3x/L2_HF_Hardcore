...
public final class SkillTreesData implements IXmlReader {
    ...
    private final Map<Integer, L2SkillLearn> masterySkillTree = new LinkedHashMap<>();
    private final Set<Integer> masterySkillIds = new HashSet<>();
    private final Map<Integer, L2SkillLearn> emergentSkillTree = new LinkedHashMap<>();
    private final Set<Integer> emergentSkillIds = new HashSet<>();
    ...
    public void load() {
        ...
        masterySkillTree.clear();
        emergentSkillTree.clear();
        ...
        masterySkillIds.clear();
        emergentSkillIds.clear();
                        ...
                        for (Node c = d.getFirstChild(); c != null; c = c.getNextSibling()) {
                            if ("skill".equalsIgnoreCase(c.getNodeName())) {
                                ...
                                switch (type) {
                                    ...
                                    case "MasteryClassSkillTree": {
                                        masterySkillTree.put(skillHashCode, skillLearn);
                                        masterySkillIds.add(skillLearn.getSkillId());
                                        break;
                                    }
                                    case "EmergentClassSkillTree": {
                                        emergentSkillTree.put(skillHashCode, skillLearn);
                                        emergentSkillIds.add(skillLearn.getSkillId());
                                        break;
                                    }
    ...
    public Pair<Integer, Integer> getEmergentAndMasteryAllowedSkills(L2PcInstance player) {
        int emergentSkillsAllowed = 0;
        int masterySkillsAllowed = 0;
        for (SubClass sub : player.getSubClasses().values()) {
            if (sub.getLevel() >= 80) {
                masterySkillsAllowed += 1;
            }
            if (sub.getLevel() >= 75) {
                emergentSkillsAllowed += 1;
            }
            if (sub.getLevel() >= 70) {
                emergentSkillsAllowed += 1;
            }
        }
        return Pair.of(emergentSkillsAllowed, masterySkillsAllowed);
    }

    public boolean isSkillEmergent(Skill skill) {
        return emergentSkillIds.contains(skill.getId());
    }

    public boolean isSkillMastery(Skill skill) {
        return masterySkillIds.contains(skill.getId());
    }

    public Pair<Integer, Integer> getEmergentAndMasteryLearnedSkills(L2PcInstance player) {
        int emergentSkillsLearned = player.getSkills().values().stream().filter(skill -> emergentSkillIds.contains(skill.getId())).mapToInt(Skill::getLevel).sum();
        int masterySkillsLearned = player.getSkills().values().stream().filter(skill -> masterySkillIds.contains(skill.getId())).mapToInt(Skill::getLevel).sum();
        return Pair.of(emergentSkillsLearned, masterySkillsLearned);
    }

    public Pair<Boolean, Boolean> getEmergentAndMasteryLearnAvailable(L2PcInstance player) {
        Pair<Integer, Integer> allowedSkillsPair = getEmergentAndMasteryAllowedSkills(player);
        int emergentSkillsAllowed = allowedSkillsPair.getLeft();
        int masterySkillsAllowed = allowedSkillsPair.getRight();

        Pair<Integer, Integer> learnedSkillsPair = getEmergentAndMasteryLearnedSkills(player);

        int emergentSkillsLearned = learnedSkillsPair.getLeft();
        int masterySkillsLearned = learnedSkillsPair.getRight();

        boolean emergentSkillLearnAvailable = emergentSkillsAllowed - emergentSkillsLearned > 0;
        boolean masterySkillLearnAvailable = masterySkillsAllowed - masterySkillsLearned > 0;
        return Pair.of(emergentSkillLearnAvailable, masterySkillLearnAvailable);
    }

    public List<L2SkillLearn> getAvailableMasterClassSkills(L2PcInstance player) {
        Pair<Boolean, Boolean> learnAvailablePair = getEmergentAndMasteryLearnAvailable(player);
        final List<L2SkillLearn> result = new ArrayList<>();
        if (learnAvailablePair.getLeft()) {
            result.addAll(availableSkillsFrom(player, emergentSkillTree));
        }
        if (learnAvailablePair.getRight()) {
            result.addAll(availableSkillsFrom(player, masterySkillTree));
        }
        return result;
    }

    private List<L2SkillLearn> availableSkillsFrom(L2PcInstance player, Map<Integer, L2SkillLearn> tree) {
        final List<L2SkillLearn> result = new ArrayList<>();
        for (L2SkillLearn skill : tree.values()) {
            if (player.getLevel() >= skill.getGetLevel()) {
                final Skill oldSkill = player.getSkills().get(skill.getSkillId());
                if (oldSkill != null) {
                    if (oldSkill.getLevel() == (skill.getSkillLevel() - 1)) {
    ...
    public L2SkillLearn getSkillLearn(AcquireSkillType skillType, int id, int lvl, L2PcInstance player) {
        ...
        switch (skillType) {
            ...
            case SUBCLASS:
                ...
                break;
            case MASTERY:
                sl = getMasteryClassSkill(id, lvl);
                break;
            case COLLECT:
    ...
    public L2SkillLearn getMasteryClassSkill(int id, int lvl) {
        int skillHash = SkillData.getSkillHashCode(id, lvl);
        L2SkillLearn learn = masterySkillTree.get(skillHash);
        if (learn == null) {
            return emergentSkillTree.get(skillHash);
        } else {
            return learn;
        }
    }
    ...
    private void report() {
        ...
        LOG.info("Loaded {} Master Class Skills.", masterySkillTree.size());
    ...
    public Set<Integer> getClassMasterySkillIds() {
        var ids = new HashSet<Integer>();
        ids.addAll(emergentSkillIds);
        ids.addAll(masterySkillIds);
        return ids;
    }
