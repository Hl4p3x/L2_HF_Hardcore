...
public class MailSystem
{
	...
	private void loadMails() {
        ...
        File file = new File(CommonConfig.DATAPACK_ROOT, "data/mail/MailList.xml");
        Document doc = null;
        if (file.exists()) {
            try {
			...
			for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
			{
				if (d.getNodeName().equals("mail")) {
                    ...
                    mailFile = new File(CommonConfig.DATAPACK_ROOT, "data/mail/" + maFile);
                    try (FileInputStream fis = new FileInputStream(mailFile);
                        ...
                        html = html.replaceAll(CommonConfig.EOL, "\n");
