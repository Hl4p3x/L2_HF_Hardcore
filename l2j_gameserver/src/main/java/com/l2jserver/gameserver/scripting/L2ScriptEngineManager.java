...
public final class L2ScriptEngineManager {
	...
	public static final File SCRIPT_FOLDER = new File(CommonConfig.DATAPACK_ROOT.getAbsolutePath(), "data/scripts");
	...
	public void reportScriptFileError(File script, ScriptException e)
	{
		...
		if (dir != null)
		{
			...
			try (FileOutputStream fos = new FileOutputStream(file)) {
				String errorHeader = "Error on: " + file.getCanonicalPath() + CommonConfig.EOL + "Line: " + e.getLineNumber() + " - Column: " + e.getColumnNumber() + CommonConfig.EOL + CommonConfig.EOL;
				...
				_log.log(Level.WARNING, "Failed executing script: " + script.getAbsolutePath() + CommonConfig.EOL + e.getMessage() + "Additionally failed when trying to write an error report on script directory. Reason: " + ioe.getMessage(), ioe);
			}
		}
		else {
			_log.log(Level.WARNING, "Failed executing script: " + script.getAbsolutePath() + CommonConfig.EOL + e.getMessage() + "Additionally failed when trying to write an error report on script directory.", e);
		}
	}
