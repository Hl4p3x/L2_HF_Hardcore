...
public final class RequestAcquireSkill extends L2GameClientPacket {
        ...
        switch (skillLearnData.getSkillType()) {
            ...
            case SUBCLASS: {
                ...
                if (subClassSkilllsQuest != null) {
                    ...
                    handleSubclassCertificationLearn(st, activeChar, trainer, skill, s);
                } else {
                    _log.warning(
                            "Cannot learn subskill because there is no SubClassSkills or Mastery Certification quest, Sub-Class skill Id: " + skillLearnData.getId() + " level: "
                                    + skillLearnData.getLevel() + " for player " + activeChar.getName() + "!");
                }
                break;
            }
            case MASTERY:
                handleMasteryCertificationLearn(activeChar, trainer, skill, s);
                break;
            case COLLECT: {
                if (SkillsHelper.checkPlayerSkill(skillLearnData, activeChar, trainer, s)) {
                    SkillsHelper.giveSkill(skillLearnData, activeChar, trainer, skill);
    ...
    private void handleMasteryCertificationLearn(L2PcInstance activeChar, L2Npc trainer, Skill skill, L2SkillLearn skillLearn) {
        boolean isSkillMastery = SkillTreesData.getInstance().isSkillMastery(skill);
        boolean isSkillEmergent = SkillTreesData.getInstance().isSkillEmergent(skill);

        var learnsAvailablePair = SkillTreesData.getInstance().getEmergentAndMasteryLearnAvailable(activeChar);

        if ((learnsAvailablePair.getLeft() && isSkillEmergent) || (learnsAvailablePair.getRight() && isSkillMastery)) {
            if (SkillsHelper.checkPlayerSkill(skillLearnData, activeChar, trainer, skillLearn)) {
                SkillsHelper.giveSkill(skillLearnData, activeChar, trainer, skill);
            }
        }
    }

    private void handleSubclassCertificationLearn(QuestState st, L2PcInstance activeChar, L2Npc trainer, Skill skill, L2SkillLearn skillLearn) {
