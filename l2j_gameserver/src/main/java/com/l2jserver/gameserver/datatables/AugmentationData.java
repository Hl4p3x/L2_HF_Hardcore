	...
	private final void load()
	{
		...
		if (!Config.RETAIL_LIKE_AUGMENTATION)
		{
			try {
				...
				File file = new File(CommonConfig.DATAPACK_ROOT + "/data/stats/augmentation/augmentation_skillmap.xml");
				if (!file.exists()) {
			...
			File aFile = new File(CommonConfig.DATAPACK_ROOT + "/data/stats/augmentation/retailchances.xml");
			if (aFile.exists()) {
				Document aDoc = null;
		...
		if (Config.RETAIL_LIKE_AUGMENTATION_ACCESSORY) {
			...
			File aFile3 = new File(CommonConfig.DATAPACK_ROOT + "/data/stats/augmentation/retailchances_accessory.xml");
			if (aFile3.exists()) {
				Document aDoc = null;
