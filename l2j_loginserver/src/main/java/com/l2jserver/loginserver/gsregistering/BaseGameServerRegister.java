...
public abstract class BaseGameServerRegister
{
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			else if (arg.equals("-u") || arg.equals("--unregister"))
			{
				...
				else
				{
					...
					{
						if (bundle != null) {
							System.out.printf(bundle.getString("wrongUnregisterArg") + CommonConfig.EOL, gsId);
						}
						System.exit(1);
					}
				}
			...
			try
			{
				if (_id < 0)
				{
					...
					if (registeredId < 0)
					{
						...
						System.out.printf(getBundle().getString("registrationOk") + CommonConfig.EOL, registeredId);
					}
				}
				else {
					System.out.printf(getBundle().getString("checkingIdInUse") + CommonConfig.EOL, _id);
					if (GameServerTable.getInstance().hasRegisteredGameServerOnId(_id)) {
						System.out.println(getBundle().getString("yes"));
						if (_force) {
							System.out.printf(getBundle().getString("forcingRegistration") + CommonConfig.EOL, _id);
							BaseGameServerRegister.unregisterGameServer(_id);
							BaseGameServerRegister.registerGameServer(_id, _outDir);
							System.out.printf(getBundle().getString("registrationOk") + CommonConfig.EOL, _id);
						}
						else if (_fallback)
						{
	...
	private static class UnregisterTask extends BaseTask
	{
		...
		@Override
		public void run() {
			System.out.printf(getBundle().getString("removingGsId") + CommonConfig.EOL, _id);
			try {
				BaseGameServerRegister.unregisterGameServer(_id);
			} catch (SQLException e) {
				showError(getBundle().getString("sqlErrorRegister"), e);
