...
public class L2Npc extends L2Character
{
	...
	public boolean hasSkillChance()
	{
		...
		boolean castSkill = (random > getTemplate().getMinSkillChance()) && (random < getTemplate().getMaxSkillChance());
		LOG.debug("{} has rolled {} between {} and {} to cast a skill", this, random, getTemplate().getMinSkillChance(), getTemplate().getMaxSkillChance());
		return castSkill;
	}
	...
	{
		return Math.max(getTemplate().getShortRangeSkillChance(), 100);
	}
	
	public int getLongRangeSkillChance()
	{
		return Math.max(getTemplate().getLongRangeSkillChance(), 100);
	}
