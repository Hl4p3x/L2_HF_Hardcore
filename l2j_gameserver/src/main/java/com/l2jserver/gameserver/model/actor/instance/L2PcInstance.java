...
import com.l2jserver.common.util.EnumIntBitmask;
...
import com.l2jserver.gameserver.model.actor.instance.helpers.DieDropChanceCalculator;
import com.l2jserver.gameserver.model.actor.instance.helpers.DieDropHelper;
...
public final class L2PcInstance extends L2Playable {
    ...
    private DieDropHelper dieDropHelper = new DieDropHelper(this);
    ...
    private void onDieDropItem(L2Character killer) {
        ...
        if (!isGM() || Config.KARMA_DROP_GM) {
            DieDropChanceCalculator dieDropChanceCalculator = new DieDropChanceCalculator(this, killer);

            ItemsByCategory itemsByCategory = getInventory().getCategorizedItems();
            itemsByCategory.shuffleAll();

            List<L2ItemInstance> randomWeaponsSlice = dieDropHelper.filterNonDroppableItemsAndSliceByLimit(
                    itemsByCategory.getWeapons(),
                    dieDropChanceCalculator.calculateWeaponDropCount()
            );
            dieDropHelper.handleDrop(randomWeaponsSlice, dieDropChanceCalculator.calculateWeaponDropChance(), killer);

            List<L2ItemInstance> randomArmorSlice = dieDropHelper.filterNonDroppableItemsAndSliceByLimit(
                    itemsByCategory.getArmor(),
                    dieDropChanceCalculator.calculateArmorDropCount()
            );
            dieDropHelper.handleDrop(randomArmorSlice, dieDropChanceCalculator.calculateArmorDropChance(), killer);

            List<L2ItemInstance> randomBagSlice = dieDropHelper.filterNonDroppableItemsAndSliceByLimit(
                    itemsByCategory.getBag(),
                    dieDropChanceCalculator.calculateBagDropCount()
            );
            dieDropHelper.handleDrop(randomBagSlice, dieDropChanceCalculator.calculateBagDropChance(), killer);
        }
    }
    ...
    public String getHtmlPrefix() {
        if (!CommonConfig.L2JMOD_MULTILANG_ENABLE) {
            return null;
        }
    ...
    public boolean setLang(String languageString) {
        ...
        if (CommonConfig.L2JMOD_MULTILANG_ENABLE) {
            if (CommonConfig.L2JMOD_MULTILANG_ALLOWED.contains(languageString)) {
                language = Language.of(languageString);
                result = true;
            } else {
                language = Language.of(CommonConfig.L2JMOD_MULTILANG_DEFAULT);
            }
