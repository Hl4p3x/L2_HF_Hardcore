...
public class ConsoleLogFormatter extends Formatter
{
	...
	public String format(LogRecord record) {
        ...
        StringUtil.append(output, "[", dateFmt.format(new Date(record.getMillis())), "] " + record.getMessage(), CommonConfig.EOL);

        if (record.getThrown() != null) {
            try {
                StringUtil.append(output, Util.getStackTrace(record.getThrown()), CommonConfig.EOL);
            } catch (Exception ex) {
            }
		}
