...
public final class Formulas {
    ...
    public static double calculateMultipliers(L2Character cha, double baseValue, double raidMultiplier,
        ...
        if (L2MonsterInstance.class.isAssignableFrom(cha.getClass()) && !cha.isRaid()) {
            ...
            Grade grade = GradeMapper.resolveGrade(monster.getLevel());
    ...
    public static double calcBackstabDamage(L2Character attacker, L2Character target, Skill skill, byte shld, boolean ss) {
        ...
        double penaltyMod = 1;
        if (target.isAttackable() && (target.getLevel() >= Config.MIN_NPC_LVL_DMG_PENALTY) && (attacker.getActingPlayer() != null) && ((target.getLevel() - attacker.getActingPlayer().getLevel()) >= 2)) {
            int lvlDiff = (target.getLevel() - attacker.getActingPlayer().getLevel() - 1);
            if (lvlDiff >= Config.NPC_SKILL_DMG_PENALTY.size()) {
                penaltyMod *= Config.NPC_SKILL_DMG_PENALTY.get(Config.NPC_SKILL_DMG_PENALTY.size() - 1);
            } else {
    ...
    private static double calculateDmgBenefit(int levelDiff, double damage) {
        if (Config.DMG_BENEFIT > 0 && levelDiff < -Config.DMG_BENEFIT_LEVEL_DIFF) {
            int levelDiffPlus = levelDiff + Config.DMG_BENEFIT_LEVEL_DIFF;
            return (damage / 100) * (Math.abs(levelDiffPlus) * Config.DMG_BENEFIT);
        } else {
            return 0D;
        }
    }

    private static double calculateDmgPenalty(int levelDiff, double damage) {
        if (Config.DMG_PENALTY > 0 && levelDiff > Config.DMG_PENALTY_LEVEL_DIFF) {
            int levelDiffMinus = levelDiff - Config.DMG_PENALTY_LEVEL_DIFF;
            return Math.max((damage / 100) * (Math.abs(levelDiffMinus) * Config.DMG_PENALTY), 1);
        } else {
            return 0D;
        }
    }
    ...
    public static final double calcMagicDam(L2Character attacker, L2Character target, Skill skill, byte shld, boolean sps, boolean bss, boolean mcrit) {
        ...
        if (target.isAttackable()) {
            ...
            if ((target.getLevel() >= Config.MIN_NPC_LVL_DMG_PENALTY) && (attacker.getActingPlayer() != null) && ((target.getLevel() - attacker.getActingPlayer().getLevel()) >= 2)) {
                int lvlDiff = (target.getLevel() - attacker.getActingPlayer().getLevel() - 1);
                if (lvlDiff >= Config.NPC_SKILL_DMG_PENALTY.size()) {
                    damage *= Config.NPC_SKILL_DMG_PENALTY.get(Config.NPC_SKILL_DMG_PENALTY.size() - 1);
                } else {
    ...
    public static final double calcMagicDam(L2CubicInstance attacker, L2Character target, Skill skill, boolean mcrit, byte shld) {
        ...
        if (target.isAttackable()) {
            ...
            if ((target.getLevel() >= Config.MIN_NPC_LVL_DMG_PENALTY) && (attacker.getOwner() != null) && ((target.getLevel() - attacker.getOwner().getLevel()) >= 2)) {
        ...
        int levelDiff = owner.getLevel() - target.getLevel();
        damage -= calculateDmgPenalty(levelDiff, damage);
        damage += calculateDmgBenefit(levelDiff, damage);
    ...
    public static boolean calcHitMiss(L2Character attacker, L2Character target) {
        ...
        accuracy += calculateAccuracyBenefit(attacker.getLevel() - target.getLevel());

        int chance = (80 + (2 * (accuracy - target.getEvasionRate(attacker)))) * 10;
    ...
    public static long calculateAccuracyBenefit(int levelDiff) {
        if (Config.ACCURACY_BENEFIT >= 0 && levelDiff < 0) {
            return Math.round(Math.abs(levelDiff) * Config.ACCURACY_BENEFIT);
        } else {
            return 0;
        }
    }
    ...
    public static boolean calcMagicSuccess(L2Character attacker, L2Character target, Skill skill) {
        ...
        if (target.isAttackable() && (target.getLevel()
                >= Config.MIN_NPC_LVL_MAGIC_PENALTY) && (attacker.getActingPlayer() != null)
                && (Math.abs(targetLevel - attacker.getActingPlayer().getLevel()) >= 3)) {
            int lvlDiff = Math.abs(targetLevel - attacker.getActingPlayer().getLevel() - 2);
    ...
    public static double calcManaDam(L2Character attacker, L2Character target, Skill skill, byte shld, boolean sps, boolean bss, boolean mcrit) {
        ...
        if (target.isAttackable()) {
            damage *= attacker.calcStat(Stats.PVE_MAGICAL_DMG, 1, null, null);
            if ((target.getLevel() >= Config.MIN_NPC_LVL_DMG_PENALTY) && (attacker.getActingPlayer() != null)
                    && (target.getLevel() - attacker.getActingPlayer().getLevel()) >= 2) {
                int lvlDiff = (target.getLevel() - attacker.getActingPlayer().getLevel() - 1);
                if (lvlDiff >= Config.NPC_SKILL_DMG_PENALTY.size()) {
                    damage *= Config.NPC_SKILL_DMG_PENALTY.get(Config.NPC_SKILL_DMG_PENALTY.size() - 1);
